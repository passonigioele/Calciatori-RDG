...
if uploaded_file:
    ...
    st.caption("Sorted by Games Won, Games Drew, MVP, and Goal Scored. Only players with one or more game played since 13-11-2025 are visible")
    filtered_players_df = players_df[players_df["Match Played"] > 0]
    columns_to_display = ["Player Name", "Match Played", "Games Won", "Games Drew", "Games Lost", "% Win", "% Lost", "Goal Difference", "Goal Scored", "Assists", "Goal/Game", "MVP", "Own Goals"]
    filtered_players_df = filtered_players_df[columns_to_display]
    columns_to_sort = ["Games Won", "Games Drew", "MVP", "Goal Scored"]
...
"""
Interactive player-stats chart, built to match the 'Team Lineups' sheet schema:
Match ID | Date | Player Name | Team (A/B) | Team Score | Team Conceded |
Result | Goals Scored | Assists | Own Goals | Unique Team ID
 
Assumes `lineups_df` is a pandas DataFrame with those columns (already loaded
in your real app.py). Paste this block wherever you build the app's charts,
...
def build_player_timeseries(lineups_df: pd.DataFrame) -> pd.DataFrame:
    """Return one row per player-per-match with running (cumulative) stats."""
    df = lineups_df.copy()
    df["Date"] = pd.to_datetime(df["Date"])
    df = df.sort_values(["Player Name", "Date", "Match ID"])
 
    df["Match Played"] = 1
    df["Win"] = (df["Result"] == "Win").astype(int)
    df["Loss"] = (df["Result"] == "Loss").astype(int)
 
    grp = df.groupby("Player Name", group_keys=False)
    df["Match Played"] = grp["Match Played"].cumsum()
    df["Cumulative Goals Scored"] = grp["Goals Scored"].cumsum()
    df["Cumulative Assists"] = grp["Assists"].cumsum()
    df["Cumulative Wins"] = grp["Win"].cumsum()
    df["Cumulative Losses"] = grp["Loss"].cumsum()
 
    return df
 
 
def render_player_chart(lineups_df: pd.DataFrame):
    ts = build_player_timeseries(lineups_df)
 
    metric_options = {
        "Match played": "Match Played",
        "Goal scored": "Goals Scored",
        "Assists": "Assists",
        "Cumulative goal scored": "Cumulative Goals Scored",
    ...
    with col1:
        metric_label = st.selectbox("Stat (Y axis)", list(metric_options.keys()),
                                     index=3)  # default: Cumulative goal scored
        metric_col = metric_options[metric_label]
 
    all_players = sorted(ts["Player Name"].unique())
    # default to top 8 by total cumulative goals, to avoid an overcrowded chart
    top_default = (
        ts.groupby("Player Name")["Cumulative Goals Scored"].max()
        .sort_values(ascending=False).head(8).index.tolist()
    )
 
    with col2:
        chosen_players = st.multiselect("Players", all_players, default=top_default)
 
    if not chosen_players:
        st.info("Select at least one player to plot.")
        return
 
    plot_df = ts[ts["Player Name"].isin(chosen_players)]
    ...
    fig = px.line(
        ...
        color="Player Name",
        markers=True,
        labels={"Date": "Match date", metric_col: metric_label, "Player Name": "Player"},
...
# render_player_chart(lineups_df)
